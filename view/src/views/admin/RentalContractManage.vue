            ...
            <el-table-column label="操作" width="320">
                <template #default="scope">
                    <div class="table-actions">
                        ...
                        <span v-if="[1, 2, 3].includes(scope.row.status)" @click="cancelContract(scope.row.id)">取消</span>
                    </div>
                </template>
            </el-table-column>
        </el-table>
...
export default {
    ...
    methods: {
        ...
        async fetchFreshData() {
            try {
                const { data, total } = await this.$axios.post('/rental-contract/list', this.queryDto);
        ...
        async cancelContract(id) {
            try {
                ...
                await this.$axios.put('/rental-contract/cancel', { id, cancelReason: value || '' });
